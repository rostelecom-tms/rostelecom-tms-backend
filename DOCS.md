## Ingestion запусков из внешних систем
...
Токен нужно добавить в `.env` (или переменные окружения контейнера):

```env
RUNS_INGESTION_TOKEN=вставьте_свой_длинный_секрет
```

После изменения переменных окружения нужно перезапустить backend.
